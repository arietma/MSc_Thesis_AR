...
# to closest in time compared to the tower footprint measurement date
...
# Important note: be sure to load in the correct tower dataset in 'alldata', 
...
# - Small correction; earlier the yeardoy (of the tower dataset/fp measurement)
...
find_ndvi_for_twr <-
  function(rasterno, ndvi_all){  
    ...
    # Main data: all tower measurements;     SPECIFY
    ...
    fp_date <- as.Date(paste0(year, '-', doy), format = "%Y-%j")
