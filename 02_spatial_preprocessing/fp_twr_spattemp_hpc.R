...
# and two MODIS variables (NDVI and EVI) in the tower footprints, in HPC. Later,
...
# (.nc), self-made functions from folder 'functions', tower measurements data 
# (.csv)

# Output: tower dataframe, with for every measurement, average NDVI, EVI, and 
# OWASIS variables (GWS and BBB). After running this script, the rows of the tower 
# dataframe should be sorted in 'dataset_sortrows.R'

# Important note: be sure to load in the correct tower dataset in the functions
# find_ndvi_for_twr and find_owasis_for_twr, and make sure that the Date column 
# is in the correct format

# Edits by arietma:
# - Changed the script so that it is run in hpc in a job array, so every observation
# is calculated in parallel (much faster, 15 mins instead of >1 day). For this 
...
#     - Important note: in Anunna, the HPC cluster used, only 10,000 jobs can be 
#     - run at the same time. Be sure to specify the job array in the slurm script 
#     - accordingly (i.e. run first for 1-10000, 10001-20000, etc.)
# - Adjusted the script for tower footprints (the original script is for airborne
# footprints)
...
fp.folder <- '/lustre/backup/WUR/ESG/rietm018/thesis/fp_rasters_all_0607/'
...
# Add empty columns to the tower data for storing the calculated data
alldata['BBB'] <- NaN
alldata['GWS'] <- NaN
alldata['OWD'] <- NaN
alldata['NDVI'] <- NaN
...
pathout <- '/lustre/backup/WUR/ESG/rietm018/thesis/tower_0607_spattemp_unsorted.csv'
...
for(i in 1:length(fp.files)){ # run loop for all footprints
  ...
  # Connect to the array id. Important note: in Anunna, the HPC cluster used, only
  # 10,000 jobs can be run at the same time. Be sure to specify the job array in 
  # the slurm script accordingly (i.e. run first for 1-10000, 10001-20000, etc.)  
if (i == slurm_task_id) { # connects to the array id
  ...
  # Name of footprint file is linked to the row in the tower dataset
  ...
  # for BBB
  mybbfile <- find_owasis_for_twr(rasterno=rasterno, owasisdf = ow.bb.df)$Owfile
  mybb <- rast(paste0(ow.bb.folder, mybbfile))
  bb_contr <- calc_spatdata_in_fp(fp=myfp, spatdata=mybb)$spatdata_contr
  
  # for GR
  mygrfile <- find_owasis_for_twr(rasterno=rasterno, owasisdf =ow.gr.df)$Owfile
  mygr <- rast(paste0(ow.gr.folder, mygrfile))
  gr_contr <- calc_spatdata_in_fp(fp=myfp, spatdata = mygr)$spatdata_contr
  
  # NDVI
  myndvifile <- find_ndvi_for_twr(rasterno=rasterno, ndvi_all=ndvi_all)
  ...
  # EVI
  myevifile <- find_ndvi_for_twr(rasterno=rasterno, ndvi_all=evi_all)
