...
# weighted peat depth  and height in tower footprint in HPC.
...
# Input: land use, soil, peat depth and ahn maps,  all tower footprint files (.nc), 
# self-made functions from folder 'functions', tower measurements data (.csv)

# Output: tower dataframe, with for every measurement, the present percentage of
# every land use and soil class, and the weighted average peat depth and height.
# of the footprint. After running this script, the rows of the tower dataframe 
...
# footprints)
# - Added the calculation of Peat Depth and ahn in the tower footprints
...
# Load tower data
alldata <- read.csv('/lustre/backup/WUR/ESG/rietm018/thesis/tower_0607_spattemp.csv', header=T, row.names='X')
...
pathout <- '/lustre/backup/WUR/ESG/rietm018/thesis/tower_0607_preprocessed_unsorted.csv'
...
if (i == slurm_task_id) { 
  ...
  # Add to the right row in the tower dataset
